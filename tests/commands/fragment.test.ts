...
describe("handleFragmentSplit", () => {
	...
	it("creates fragment files in the sync repo", async () => {
		...
		const tddFrag = await fs.readFile(path.join(syncRepoDir, "shared/standards/tdd.md"), "utf-8");
	...
	it("creates the index file at claude/CLAUDE.md in the sync repo", async () => {
		...
		const indexContent = await fs.readFile(path.join(syncRepoDir, "claude", "CLAUDE.md"), "utf-8");
	...
	it.skipIf(!canCreateSymlinks)(
		"replaces CLAUDE.md with a symlink pointing at the sync repo index",
		async () => {
			const { syncRepoDir } = await setupV2SyncRepo(tmpDir);
			const claudeDir = await createClaudeDir(tmpDir, SAMPLE_CLAUDE_MD);
	...
	it.skipIf(!canCreateSymlinks)(
		"is idempotent — skips when CLAUDE.md is already a symlink",
		async () => {
			const { syncRepoDir } = await setupV2SyncRepo(tmpDir);
			const claudeDir = await createClaudeDir(tmpDir, SAMPLE_CLAUDE_MD);
			const claudeMdPath = path.join(claudeDir, "CLAUDE.md");
...
describe("handleFragmentAdd", () => {
	...
	it("adds a new @-reference to the index", async () => {
		...
		const result = await fs.readFile(path.join(syncRepoDir, "claude", "CLAUDE.md"), "utf-8");
...
describe("registerFragmentCommand (CLI)", () => {
	...
	it.skipIf(!canCreateSymlinks)("fragment split command is registered and reachable", async () => {
		const { syncRepoDir } = await setupV2SyncRepo(tmpDir);
	...
	it("fragment status command is registered and reachable", async () => {
		...
		const program = createProgram();
		await program.parseAsync(["node", "test", "fragment", "status", "--repo-path", syncRepoDir]);
